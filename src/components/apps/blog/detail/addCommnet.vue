...
<template>
    <div v-if="comments">
        <v-card variant="flat" class="mb-5 pa-5 border-light bg-grey100">
            ...
            <div class="my-1 d-flex align-center gap-3">
                <v-tooltip text="Reply">
                    <template v-slot:activator="{ props }">
                        <v-btn icon color="primary" variant="flat" v-bind="props" size="x-small">
        ...
        <template v-for="(reply, i) in comments.replies" :key="reply.id">
            <div class="ml-10">
                <v-card variant="flat" class="mb-5 pa-5 border-light bg-grey100" v-if="reply">
